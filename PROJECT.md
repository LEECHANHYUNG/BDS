...
- 현재 목표: 린트/포맷 노선을 Biome → **oxlint + oxfmt**로 전환(ADR-0008). 코드 0줄인 지금이 전환 비용 최저점이라는 판단 + 현업 oxc 운영 대비 학습 동기. 이후 `@bds/tokens`·`@bds/react` 첫 패키지 추가로 복귀.
- 관련 산출물: `docs/research/lint-format-toolchain.md`, `docs/plans/lint-format-toolchain.md`, `CONTEXT.md`, `docs/adr/0001~0007`, `DESIGN.md`
...
- **(2026-06-20) CodeRabbit 리뷰 설정 확정** (그릴링): 언어 `ko-KR`; 프로파일 `chill`(Biome과 신호 중복 회피, 설계·도메인 위험 집중); `request_changes_workflow: false`(머지 비차단, 조언자); 도구 Biome on·gitleaks on·markdownlint off; `auto_review.drafts: false`; `knowledge_base` 학습 유지; `path_instructions`는 실재 경로(`docs/**`·`.changeset/**`)에만, `packages/**`는 첫 컴포넌트 시점 보류. 근거·재평가 트리거(CI 도입 시 request_changes→true, 첫 컴포넌트 시 packages 관점 추가)는 ADR-0007.
