...
React 디자인 시스템 모노레포입니다. 현재는 컴포넌트를 추가하기 전, 기반 환경(워크스페이스 골격 · 코드 품질 · 버저닝)만 구성된 단계입니다.
...
`.claude/skills/`에 정의된 세 스킬을 기반으로 **조사 → 계획 → 실행** 3단계로 작업합니다.
...
3. **실행 (implement)** — `implement-phase` 스킬
   승인된 계획을 Codex에 핸드오프하고, Claude Code는 설계 의도·접근성·파급 영향을 리뷰합니다. 구현은 작업마다 타입체크·테스트로 검증하고, 계획서에 완료 표시(`[x]`)를 남기며, 논리 단위로 커밋합니다. (역할 분담은 아래 [협업 구조](#두-ai의-협업-구조-claude-code--codex) 참고)
