...
| 디자인 시스템 맥락 | 토큰 체계 설계, 컴포넌트 API 설계, 접근성·금융 규제 검토, 변경의 파급 영향 판단, PR 리뷰 | 컴포넌트 구현, 토큰 빌드 파이프라인 실행, Storybook/문서 사이트 생성, 시각 회귀 스냅샷, 데모 캡처 |
...
| 리스크 노트 | Claude Code | 추정 금지 영역, 접근성·금융 규제 위험, 과장 표현 |
